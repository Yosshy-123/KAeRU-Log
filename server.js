...
function logUserAction(clientId, action, extra = {}) {
    ...
    const extraStr = Object.keys(info).length
        ? ` ${JSON.stringify(info)}`
        : '';

    console.log(
        `[${time}] [User:${clientId}]${username} Action: ${action}${extraStr}`
    );
}
...
io.use((socket, next) => {
    ...
    if (headers['token-key'] !== TOKEN_KEY) {
...
app.use((req, res, next) => {
    if (req.headers['token-key'] !== TOKEN_KEY) {
...
async function monthlyRedisReset(io) {
    ...
    const currentMonth =
        `${jstNow.getFullYear()}-${String(jstNow.getMonth() + 1).padStart(2, '0')}`;
...
app.post('/api/messages', async (req, res) => {
    ...
    let intervalCount =
        Number(await redisClient.get(repeatIntervalKey)) || 0;

    if (lastMessage === message && lastTime) {
        const interval     = now - Number(lastTime);
        const lastInterval =
            Number(await redisClient.get(`msg:last_interval:${clientId}`)) || 0;
...
app.post('/api/clear', async (req, res) => {
    ...
    const username =
        (await redisClient.get(`username:${clientId}`)) || 'unknown';
